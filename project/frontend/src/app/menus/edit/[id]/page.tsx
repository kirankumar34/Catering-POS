...
const EditMenuPackagePage: React.FC<PageProps> = ({ params }) => {
  ...
  const onSubmit = async (values: MenuFormValues) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.data) {
        setApiError(err.response.data.message || 'Failed to update package details.');
      } else {
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-slate-950 text-white gap-3">
        ...
        <span className="text-sm font-medium tracking-wide">Loading Package Parameters...</span>
      </div>
    );
  }
  ...
  // Map package data items into raw GUID array for default values mapping
  const mappedInitialValues = {
    ...menuData,
    itemIds: menuData.items ? menuData.items.map((item: { id: string }) => item.id) : [],
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-6 md:p-8 space-y-8 overflow-y-auto">
        <MenuPackageForm
          title={`Edit Package: ${menuData.name}`}
...
export default EditMenuPackagePage;
